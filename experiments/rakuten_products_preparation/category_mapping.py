...
from sampytools.pandas_utils import create_new_col_based_on_dict, convert_columns_to_lowercase_and_nowhitespace
...
unmapped_orig_categories=get_list_diff(orig_categories,cmdf["original_category"].unique())
logging.info(f"Number of new categories : {cmdf['mapped_category'].nunique()}")
logging.info(f"Unmapped original categories {len(unmapped_orig_categories)} : {unmapped_orig_categories}")
...
logging.info(f"Saved product categories with {pcdf['category'].nunique()} new categories to {data_folder/new_product_category_filename}")